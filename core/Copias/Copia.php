<?php
/**
 * AxiDB - Copias\Copia: hacer una copia, completa o incremental.
 *
...
final class Copia
{
    ...
    public static function hacer(string $base, string $carpeta, ?string $desde = null): array
    {
        ...
        $inventario = Inventario::de($base);
        $huellas    = Inventario::huellas($inventario);

        $anteriores = $desde === null ? [] : self::huellasDe($desde);
        $tipo       = $desde === null ? self::COMPLETA : self::INCREMENTAL;

        $id       = self::nuevoId($tipo);
        $destino  = \rtrim($carpeta, '/\\') . '/' . $id . Catalogo::EXTENSION;
        $cabecera = [
            'version' => 1,
            'id'      => $id,
            'momento' => \date('c'),
            'tipo'    => $tipo,
            'base'    => $desde === null ? null : Contenedor::cabecera($desde)['id'],
            'huellas' => $huellas,
        ];

        $contenedor = Contenedor::crear($destino, $cabecera);
        $guardados  = 0;
    ...
    private static function huellasDe(string $copia): array
    {
        ...
        $huellas = Contenedor::cabecera($copia)['huellas'] ?? [];
