<?php
/**
 * AxiDB - Copias\Restauracion: devolver los datos a como estaban.
 *
...
final class Restauracion
{
    ...
    public static function hacer(string $copia, string $destino, ?string $carpeta = null): array
    {
        ...
        $ultima = Contenedor::cabecera($copia);
        ...
        foreach ($cadena as $eslabon) {
            Contenedor::recorrer($eslabon, static function (string $ruta, string $bytes) use (&$archivos): void {
                $archivos[$ruta] = $bytes;
            });
        }
    ...
    private static function cadena(string $copia, string $carpeta): array
    {
        ...
        while (true) {
            ...
            $cabecera = Contenedor::cabecera($actual);
            ...
            $actual = Catalogo::buscar($carpeta, (string) $base);
        }
    ...
    private static function quitarLosQueSobran(string $destino, array $esperados): int
    {
        ...
        foreach (Inventario::de($destino) as $ruta => $entrada) {
