<?php
/**
 * AxiDB - Sql\Lectura: ejecuta SELECT y COUNT.
 *
...
final class Lectura
{
    ...
    {
        if (isset($ast['vector'])) {
            return (new Vectorial($this->db))->ejecutar($ast, $explicar);
        }
        ...
        if ($deVista !== null) {
            ...
            return Resultado::construir($deVista, $ast);
        }
         ...
         */
        return Resultado::construir($q->get(), $ast);
    }
    ...
    private function construirConsulta(array $ast): Query
    {
        ...
        return $this->db->find($ast['collection'])
            ->whereExpr((new Subconsultas($this->db))->resolver($ast['where_expr']));
    }
    ...
    private function conCruce(array $ast, bool $explicar): mixed
    {
        ...
        $donde = (new Subconsultas($this->db))->resolver($ast['where_expr']);
        if ($donde !== null) {
        ...
        return Resultado::construir($filas, $ast);
    }
    ...
    {
        $sql = Estructura::sqlDeVista($this->db, (string) ($ast['collection'] ?? ''));
        if ($sql === null) {
            return null;
        }
