<?php
/**
 * AxiDB - Sql\Resultado: de documentos filtrados a las filas que se devuelven.
 *
...
final class Resultado
{
    ...
    {
        $pares = [];
        foreach (Agrupacion::hacer($documentos, $ast['group_by'] ?? [], $agregados) as $grupo) {
    ...
    private static function proyectar(array $doc, array $proyeccion, array $agregados): array
    {
        ...
        foreach ($proyeccion as $item) {
            ...
            $nombre = $item['alias'] ?? Valor::firma($item['expr']);
            $fila[$nombre] = Valor::de($item['expr'], $doc, $agregados);
        }
        return $fila;
    }
