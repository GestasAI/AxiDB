...
foreach (['fs', 'packed'] as $driver) {
    section("Crypto sobre el driver {$driver}");
    ...
    eq('cifrar reescribe el documento que ya habia', 1, $db->encrypt('fichas'));
    ok('la coleccion queda marcada como cifrada', $db->isEncrypted('fichas'));
    ...
    throws('con la clave equivocada, tambien',
        static fn () => (new Db($dir, ['durable' => false, 'key' => 'otra distinta']))->get('fichas', 'f2'));
...
ok('y lo que hay guardado es un bloque cerrado, no el texto', Box::esBloque($doc['_cif'] ?? null));
...
/* ─────────────────────────────────────────────────────────────────────────── */
section('Crypto y vectores no se mezclan');

$dir = tmpdir('cifrado_vectores');
$db  = new Db($dir, ['durable' => false, 'key' => CLAVE]);
$db->encrypt('notas');
throws('no se activan vectores sobre una coleccion cifrada',
    static fn () => $db->enableVectors('notas'));

$db->enableVectors('libres');
