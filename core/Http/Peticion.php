<?php
/**
 * AxiDB - Http\Peticion: lo que llega, ya comprobado.
 *
...
final class Peticion
{
    ...
    public static function desde(array $server, ?string $crudo = null): self
    {
        ...
        if ($declarado > self::LIMITE_BYTES) {
            throw new PeticionInvalida('El cuerpo supera el limite de ' . self::LIMITE_BYTES . ' bytes.', 413);
        }
    ...
    {
        if (\trim($crudo) === '') {
            throw new PeticionInvalida('Falta el cuerpo de la peticion.', 400);
        }
        $datos = \json_decode($crudo, true);
        if (!\is_array($datos)) {
            throw new PeticionInvalida('El cuerpo no es JSON valido: ' . \json_last_error_msg(), 400);
        }
        if (\array_is_list($datos)) {
            throw new PeticionInvalida('El cuerpo debe ser un objeto JSON, no una lista.', 400);
        }
        return $datos;
    }
