<?php
/**
 * AxiDB - Http\Puente: de una accion JSON a una llamada al motor.
 *
...
final class Puente
{
    ...
    public function atender(Peticion $p): Respuesta
    {
        $accion = $p->texto('accion');
        if ($accion === null || !\in_array($accion, self::ACCIONES, true)) {
            return Respuesta::mal(400, 'Accion desconocida. Admitidas: ' . \implode(', ', self::ACCIONES) . '.');
        }

        $ast = $accion === 'sql' ? $this->analizar($p) : null;

        $coleccion = $ast !== null ? (string) ($ast['collection'] ?? '') : (string) $p->texto('coleccion');
        $escribe   = $ast !== null ? self::sqlEscribe($ast) : Permisos::escribe($accion);

        if ($coleccion === '') {
            return Respuesta::mal(400, "Falta 'coleccion'.");
        }

        [$vale, $codigo, $motivo] = $this->permisos->decide($escribe, $coleccion, $p->token, $p->esLocal);
        if (!$vale) {
            return Respuesta::mal($codigo, $motivo);
        }

        return Respuesta::bien($this->ejecutar($accion, $coleccion, $p, $ast));
    }
     ...
     *
     * @throws PeticionInvalida
     */
    private function analizar(Peticion $p): array
    {
        $sentencia = $p->texto('sentencia');
        if ($sentencia === null || \trim($sentencia) === '') {
            throw new PeticionInvalida("Falta 'sentencia'.");
        }
        try {
            ...
            throw new PeticionInvalida($e->getMessage());
        }
    }
    ...
    private function ejecutar(string $accion, string $coleccion, Peticion $p, ?array $ast): mixed
    {
        return match ($accion) {
    ...
    private function consulta(string $coleccion, Peticion $p): Query
    {
        $q = $this->db->find($coleccion);

        foreach ((array) ($p->cuerpo['donde'] ?? []) as $condicion) {
            if (!\is_array($condicion) || \count($condicion) < 2) {
                throw new PeticionInvalida("Cada condicion de 'donde' es [campo, operador, valor].");
            }
    ...
    private function id(Peticion $p): string
    {
        $id = $p->texto('id');
        if ($id === null || $id === '') {
            throw new PeticionInvalida("Falta 'id'.");
        }
        return $id;
    }

    private function datos(Peticion $p): array
    {
        $datos = $p->cuerpo['datos'] ?? null;
        if (!\is_array($datos) || \array_is_list($datos)) {
            throw new PeticionInvalida("'datos' debe ser un objeto JSON.");
        }
        return $datos;
    }
}
