...
final class Health
{
    ...
    public function stats(string $coleccion): array
    {
        ...
        return [
            ...
            'durabilidad'      => $almacen->durabilityOf($coleccion),
            ...
            'bytes'            => self::weightOf($dir),
            'proporcionMuerta' => $almacen->deadRatio($coleccion),
        ];
    }
    ...
    public function checkup(): array
    {
        ...
        foreach ($colecciones as $coleccion) {
            ...
            $avisos = [...$avisos, ...$this->warningsOf($coleccion, $e)];
        }
    ...
    private function warningsOf(string $coleccion, array $e): array
    {
        $avisos = [];

        if ($e['proporcionMuerta'] > self::BASURA_AVISO) {
            $avisos[] = self::warning($coleccion, 'atencion',
        ...
        foreach ($this->db->verifyIndexes($coleccion) as $campo => $revision) {
            if (!empty($revision['ilegible'])) {
                $avisos[] = self::warning($coleccion, 'grave',
     ...
     */
    private static function warning(string $coleccion, string $gravedad, string $que, string $hacer): array
    {
        return ['coleccion' => $coleccion, 'gravedad' => $gravedad, 'que' => $que, 'hacer' => $hacer];
    }

    /** Lo que ocupa un directorio con todo lo que lleva dentro. */
    private static function weightOf(string $dir): int
    {
        if (!\is_dir($dir)) {
            return 0;
        }
